...
True argmax - 0.25
"""
tf1(x:: Float64)::Float64 = -(x - 0.25)^2 + 1 
...
"""
tf2(x:: Float64)::Float64 = sin(10 * x) + 0.2 * x
...
True argmax - 0.7
"""
tf3(x:: Float64)::Float64 = exp(-10000 * (x - 0.7)^2)
...
"""
tf4(x:: Float64)::Float64 = exp(-50 * (x - 0.5)^2) + 0.8 * exp(-10 * (x - 0.075)^2)
...
True argmax - 0.7
"""
tf5(x:: Float64)::Float64 = -abs(x - 0.7) - 1
...
"""
tf7(x:: Float64)::Float64 = exp(-10000 * (x - 0.5)^2) + 0.5 * sin(50 * x)
...
"""
tf8(x:: Float64)::Float64 = exp(-(10 * x - 1)^2) + exp(-(10 * x - 5)^2) + exp(-(10 * x - 9)^2) + x/10000
...
"""
tf9(x:: Float64)::Float64 = sin(100x) * exp(-0.5(x - 0.5)^2) + 0.01x

"""
Linear increase that has a jump discontinuit followed by a local quadratic max
True argmax = 0.4
"""
function tf10(x::Float64)::Float64 
   if x < 0.4
        return x
    else
        return -(x-0.6)^2 - 0.2
...
True argmax = 0.8
"""
function tf11(x::Float64)::Float64
    if x < 0.2
        return x
    elseif x <= 0.4
        return 0.2
...
True argmax = 0.5 
"""
function tf12(x::Float64)::Float64
    if x < 0.3
        return x
    elseif x <= 0.7
...
"""
function make_test_func(denom:: Int; x1=pi/10, x2=pi/5, k=5000.0)::Function
...
"""
function gen_random_points(; min_distance:: Float64 = 0.1)::Tuple{Float64, Float64}
